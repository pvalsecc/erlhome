...
%%%
%%% @end
%%% Created : 20. Apr 2015 11:04 AM
%%%-------------------------------------------------------------------
-module(ehome_event_forwarder).
-author("pvalsecc").
...
-spec(init(InitArgs :: term()) ->
    ...
    {error, Reason :: term()}).
init({Module, Target}) ->
    {ok, #state{module = Module, target = Target}}.
...
-spec(handle_event(Event :: term(), State :: #state{}) ->
    ...
    remove_handler).
handle_event(Event, #state{module = Module, target = Target} = State) ->
    Module:handle_event(Target, Event),
    {ok, State}.
